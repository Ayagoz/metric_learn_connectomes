...
param_grid = {'LMNN': {'metric__max_iter':[500],
                       'metric__min_iter':[10],
                  'metric__learn_rate': [1e-7, 1e-2],
                  'metric__regularization': [0.1, 0.25, 0.65, 0.9],
                  'metric__conv_tol': [0.001, 1e-5],
                  'metric__k': [25, 55],
                  'metric__alpha': [1e-7, 1e-5, 1e-3,1,10],
...
table = pipeliner.get_results(X, y, scoring=['roc_auc'], caching_steps = ['normalization1','normalization2', 'features'], logs_file = 'ucla_results_LMNN_exp_comp.log')
print('finish')
table.to_csv('ucla_results_LMNN_exp_comp', sep = '\t')